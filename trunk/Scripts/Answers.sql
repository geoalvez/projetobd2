...
(
	...
	CURSOR C IS
		SELECT c.cliente.nome as NOME_CLIENTE,
			  ...
			  WHERE C.CLIENTE.CPF = CPF_C and e.data_emprestimo > DATA_P
...
BEGIN
	...
	LOOP
		...
		dbms_output.put_line('NOME CLIENTE: ' || CP.NOME_CLIENTE || ' DATA EMPR.: ' || CP.DATA_EMPRESTIMO || 
		                        ' DATA ENTREGA: ' || CP.DATA_DE_ENTREGA || ' VALOR DA MULTA: ' || CP.VALOR_DA_MULTA || 
								' MULTA PAGA: ' || CP.MULTA_PAGA || ' FILME EMPRESTADO: ' || CP.TITULO_FILME);
	END LOOP;
	CLOSE C;
END;
